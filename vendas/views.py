...
from financeiro.models import FormaPagamento, Movimentacao, Categoria, models, ConfiguracaoCashback
...
def salvar_venda(request):
    ...
    try:
        ...
        quitar_divida_na_venda = request.POST.get('quitar_divida_na_venda') == 'true'
        ...
        data_venda = (
            # ⭐️ CORREÇÃO: Converte a string do formulário para um objeto datetime "ciente" do fuso horário local
            timezone.make_aware(
                datetime.strptime(data_venda_str, '%Y-%m-%dT%H:%M'),
        ...
        # Carrega as configurações de cashback do admin
        config_cashback, _ = ConfiguracaoCashback.objects.get_or_create(pk=1)

        valor_recebido_liquido = valor_total - valor_cashback_utilizado
        valor_cashback_gerado = Decimal('0.00')
        if config_cashback.cashback_ativo:
            taxa = config_cashback.percentual_cashback / Decimal('100')
            valor_cashback_gerado = valor_total * taxa
        ...
        with transaction.atomic():
            ...
            if cliente:
                ...
                # ⭐️ NOVO: Lógica para quitar a dívida se o sinalizador estiver ativo
                if quitar_divida_na_venda:
                    dividas_abertas = cliente.divida_set.filter(pago=False).order_by('pk')
                    valor_a_quitar = sum(d.valor_pendente for d in dividas_abertas)

                    for divida in dividas_abertas:
                        divida.valor_pendente = Decimal('0.00')
                        divida.pago = True
                        divida.save()
                    
                    # ⭐️ CORREÇÃO: Recalcula o total da dívida a partir da fonte da verdade.
                    # Como quitamos tudo, o novo total deve ser zero.
                    novo_total_divida = cliente.divida_set.filter(pago=False).aggregate(total=models.Sum('valor_pendente'))['total'] or Decimal('0.00')
                    cliente.divida_total_db = novo_total_divida
                    
                    # Adiciona uma observação na venda para registrar o ato
                    venda.refresh_from_db() # Garante que o objeto 'venda' está atualizado
                    venda.observacao = (venda.observacao or '') + f"\n[INFO] Quitou dívida de R$ {valor_a_quitar:.2f} nesta venda."
...
def lancamento_vendas_view(request):
    ...
    if request.method == 'POST':
        ...
        # ⭐️ CORREÇÃO: Converte a hora atual para o fuso horário do Brasil (São Paulo)
        agora_no_brasil = timezone.localtime(timezone.now())

        data_atual = agora_no_brasil.strftime('%Y-%m-%d')
        data_hora_atual = agora_no_brasil.strftime('%Y-%m-%dT%H:%M')
